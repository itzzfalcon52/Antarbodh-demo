...
def get_profile(
    ...
    """
    Return an ANTARBODH temperature profile at a requested location.

    Historical:
        Reads the cached 2025 ANTARBODH field.

    Predict:
        Runs the frozen CNN using the operational surface inputs.
    ...
    if mode == "historical":
        ...
        result = historical_service.get_profile(
            date,
            lat,
            lon,
        )
        ...
        if result is None:
            raise HTTPException(
                status_code=500,
                detail="Historical profile could not be loaded.",
            )

        return result

    # ---------------------------------------------------------
    # Auto → historical when available
    # ---------------------------------------------------------
    ...
    # Interpolate to the requested location rather than
    # pretending the nearest grid cell is the exact location.
    try:
